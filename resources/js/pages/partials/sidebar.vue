    ...
    <div class="left_col scroll-view">
      <div class="navbar nav_title" style="border: 0;">
          <router-link :to="{ name: 'dashboard' }" class="site_title">
            <i class="fa bx bx-cart"></i><span class="fw-bolder ms-2  text-capitalize" v-if="companyInfo">{{ companyInfo.company_name }}</span>
            </router-link>
      </div>
      <br />
      <!-- sidebar menu -->
      <div id="sidebar-menu" class="main_menu_side hidden-print main_menu mt-5">
        <div class="menu_section">
          <ul class="nav side-menu">
            <li :class="$route.name === 'dashboard' ? 'active' : ''" ><router-link
                :to="{ name: 'dashboard' }" class="menu-link">
                <i class="fa bx bx-home"></i>
                {{ $t('trans.dashboard') }}
              </router-link></li>
            <li v-if="hasPermission('user') || hasPermission('user.index') || hasPermission('user.create') || hasPermission('user.edit') || hasPermission('user.delete')">
              <a><i class="fa bx bx-user"></i> {{ $t('trans.Users') }} <span class="bx bx-chevron-down float-right"></span></a>
              <ul class="nav child_menu">
                <li v-if="hasPermission('user.create')" class="menu-item"
                  ...
                  <router-link :to="{ name: 'user.create' }" class="menu-link"> <i
                      class="menu-icon tf-icons bx bx-add-to-queue"></i>{{ $t('trans.User') }} {{ $t('trans.Add')
                      }}</router-link>
                </li>
                <li v-if="hasPermission('user.index') || hasPermission('user.edit') || hasPermission('user.delete')"
                  ...
                  <router-link :to="{ name: 'user.index' }" class="menu-link"> <i
                      class="menu-icon tf-icons bx bx-list-ol"></i>{{ $t('trans.User') }} {{ $t('trans.List')
                      }}</router-link>
                </li>
            ...
            <li>
              <a href="javascript:void(0);" >
                ...
                {{ $t('trans.Settings') }} <span class="bx bx-chevron-down float-right"></span>
              </a>
              <ul class="nav child_menu">
                <li class="menu-item" :class="$route.name === 'balance.index' ? 'current-page' : ''">
                  <router-link :to="{ name: 'language.index' }" class="menu-link">
...
export default defineComponent({
  ...
  mounted() {
    ...
    script.src = '/backend/admin1/js/custom.js';
    script.async = true;
